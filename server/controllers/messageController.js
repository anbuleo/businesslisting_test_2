...
const sendMessage = async (req, res) => {
  try {
    ...
    // Validate booking exists and employee has access
    const booking = await Booking.findById(bookingId);
    if (!booking) {
      return res.status(404).json({ message: 'Booking not found' });
    }

    // Check if employee is part of this booking
    if (booking.assignedEmployee.toString() !== req.employee.id) {
    ...
    const message = await Message.create({
      ...
      recipient: {
        id: recipientId,
        type: recipientType,
        name: recipientType === 'user' ? booking.customerContact?.name || 'User' : 'Support'
      },
      content,
      messageType,
      metadata
