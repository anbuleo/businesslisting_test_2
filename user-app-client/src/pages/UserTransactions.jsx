...
const UserTransactions = () => {
  const { getUserWalletTransactions } = useAuth();
  ...
  const fetchTransactions = async () => {
    setLoading(true);
    const result = await getUserWalletTransactions(filters.page, 20, filters.type, filters.status);
  ...
  const getTransactionIcon = (type) => {
    switch (type) {
      ...
      case 'payment': return <ArrowUpCircle className="w-5 h-5 text-red-600" />;
      case 'refund': return <ArrowDownCircle className="w-5 h-5 text-blue-600" />;
      case 'adjustment': return <Calendar className="w-5 h-5 text-gray-600" />;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">My Wallet Transactions</h1>
          <p className="mt-2 text-gray-600">
            View all your wallet deposits and payments
          </p>
        </div>
                        ...
                        <div>
                          ...
                          {transaction.bookingId && (
                            <div className="text-xs text-blue-600">
                              Booking: {transaction.bookingId.bookingId} - {transaction.bookingId.serviceDetails?.name}
                            </div>
                          )}
                        </div>
                      </div>
