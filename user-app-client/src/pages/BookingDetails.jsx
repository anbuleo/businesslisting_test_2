...
const BookingDetails = () => {
  ...
  const [showTracker, setShowTracker] = useState(false); // For employee tracker
  const [showVoiceCall, setShowVoiceCall] = useState(false);
  const [socket, setSocket] = useState(null);

  useEffect(() => {
    // Initialize socket connection
    const newSocket = io('http://localhost:5001'); // Connect to user app backend
    setSocket(newSocket);

    // Listen for incoming calls (from employee app)
    ...
    // Listen for call responses (e.g., employee accepted/declined)
  ...
  const handleInitiateCall = (socketInstance, callType) => {
    if (!booking.employeeDetails?.name || !booking.assignedEmployee) {
      setError('Employee details missing for call.');
      return;
    }
    socketInstance.emit('initiate_call', {
      bookingId: booking._id, callType, callerInfo: { id: user.id, name: user.name, type: 'user' }
    });
    setShowVoiceCall(true);
  };
  ...
  return (
    <div className="space-y-6">
            ...
            <div className="space-y-3">
              {booking.employeeDetails && booking.status !== 'completed' && booking.status !== 'cancelled' && (
                <>
                  ...
                  <button 
                    onClick={() => setShowTracker(true)} // This button opens the tracker modal
                    className="w-full btn-secondary flex items-center justify-center space-x-2"
                  >
      ...
      {showTracker && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="w-full max-w-md mx-4">
            {/* <EmployeeTracker booking={booking} /> */} {/* Re-enable when EmployeeTracker is fully implemented */}
